...
set val(rxPower)	 35.28e-3				   ;# receive power consumption 
set val(txPower) 	 31.32e-3	   		;# transmit power consumption
set val(initialEnergy)   1000				;# initial energy 
set val(idlePower)	 712e-6			; #idle power 
set val(sleepPower)	144e-9			; #sleep power 
...
proc finish {} {
    ...
    if { $val(execawk) == 1 } {
        if { $val(transportProtocol) == "udp" } {
            exec awk -v exp_packet_type=cbr -v initial_energy=$val(initialEnergy) -f parse.awk mysim_trace.tr & 
        } elseif { $val(transportProtocol) == "tcp" } {
            exec awk -v exp_packet_type=tcp -v initial_energy=$val(initialEnergy) -f parse.awk mysim_trace.tr & 
        }
    }
}
